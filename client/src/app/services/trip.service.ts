...
export class TripService {
    ...
    update(request: TripRequest, tripId: number): Observable<Trip>{
        return this.http.put<Trip>(`${this.apiUrl}/${tripId}`, request, {
            headers: {
                'X-User-Id': `${this.authService.getCurrentUser()?.id}`
        }});
    }
    ...
    reorderActivities(request: ReorderRequest, tripId: number): Observable<any>{
        return this.http.put(`${this.apiUrl}/${tripId}/activities/reorder`, request, {
            headers: {
                'X-User-Id': `${this.authService.getCurrentUser()?.id}`
        }});
    }
    ...
    createActivity(tripId: number, request: ActivityCreateRequest): Observable<Activity>{
        return this.http.post<Activity>(`${this.apiUrl}/${tripId}/activities`, request, {
            headers: {
                'X-User-Id': `${this.authService.getCurrentUser()?.id}`
        }});
    }
